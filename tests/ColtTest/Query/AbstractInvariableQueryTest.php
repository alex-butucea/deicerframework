...
use ColtTest\Query\TestableInvariableQueryWithValidFetchData;
use ColtTest\Query\TestableInvariableQueryWithExceptionThrowingFetchData;

/**
 * Colt Invariable Query unit test suite
 * 
 * @category   ColtTest
 * @package    Query
 * @version    $id$
...
class AbstractInvariableQueryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUpEventBuilder($topic, $content)
    {
        ...
        $this->eventBuilder
            ->expects($this->once())
            ->method('withPublisher')
            ->with($this->isInstanceOf('Colt\Query\InvariableQueryInterface'))
            ->will($this->returnSelf());
        $this->eventBuilder
            ->expects($this->once())
            ->method('build')
    ...
    {
        $fooEvent = $this->getMock('Colt\Stdlib\Pubsub\EventInterface');
        $barEvent = $this->getMock('Colt\Stdlib\Pubsub\EventInterface');
        $bazEvent = $this->getMock('Colt\Stdlib\Pubsub\EventInterface');
    ...
    {
        $this->setExpectedException('Colt\Query\Exception\DataTypeException');
    ...
    public function testDecorateImplementsFluentInterface()
    {
        $mock = $this->fixture->decorate(
            $this->getMock('Colt\Query\InvariableQueryInterface')
        );
    ...
    {
        $this->setExpectedException('Colt\Query\Exception\ModelHydratorException');
    ...
    {
        $this->setExpectedException('Colt\Query\Exception\DataFetchException');
    ...
    public function testExecuteNotifiesSubscribersOfFallbackDueToModelHydratorFailure()
    {
        ...
        $fixture->subscribe($this->subscriber, $topic);
        $fixture->decorate($this->getMock('Colt\Query\InvariableQueryInterface'));
        $fixture->execute();
    }
